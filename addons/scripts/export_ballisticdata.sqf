...
// Bullet Caliber: Get the number directly, as it's not an array. 
...
// Format the data you want to copy into a single string. 
// You can use the format command for this. 
private _outputString = format [ 
    "--- ATrag Preset Script: Processing Raw Data ---\n" + 
    "Raw Caliber (mm): %1\n" + 
    "Final Caliber (cm): %2\n" + 
    "Final Muzzle Velocity (avg): %3\n" + 
    "Final BC (avg): %4\n" + 
    "Final Preset Array: %5", 
    _caliber, 
    _caliber_cm, 
    _muzzleVel_avg, 
    _bc_avg, 
    str _newPreset // The 'str' command converts the array to a string 
]; 
 
// Use the copyToClipboard command to put the formatted string on the clipboard 
copyToClipboard _outputString; 
 
// Optionally, you can still have a hint message for in-game confirmation 
hint format ["Data copied to clipboard. Check the ATrag UI to confirm."]; 
